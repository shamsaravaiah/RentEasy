-- Run this in Supabase Dashboard → SQL Editor to fix "Invalid invite link" errors.
-- Copy the entire file and click "Run".

-- 1. Ensure token column is text
DO $$
BEGIN
  IF (
    SELECT data_type
...
-- 2. Drop all accept_invite overloads
DROP FUNCTION IF EXISTS public.accept_invite(text);
DROP FUNCTION IF EXISTS public.accept_invite(text, text, text, text);

-- 3. Create accept_invite with text-only token comparison
...
-- 4. get_invite_by_token with text-only comparison
